    ...
    if let Some(content_value) = event.get_mut("content") {
        ...
        for key in map.clone().keys() {
            match event_type.as_ref() {
                "m.room.member" if key == "membership" => {}
                "m.room.create" if key == "creator" => {}
                "m.room.join_rules" if key == "join_rules" => {}
                "m.room.power_levels" if ALLOWED_POWER_LEVELS_KEYS.contains(&key.as_ref()) => {}
                "m.room.aliases" if key == "aliases" => {}
                "m.room.history_visibility" if key == "history_visibility" => {}
                _ => {
                    map.remove(key);
                }
            };
